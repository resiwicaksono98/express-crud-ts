...
describe("POST /api/contracts/:contactId/addresses", () => {
  beforeEach(async () => {
  ...
  it("should be able to create address", async () => {
    ...
    expect(response.body.data.province).toBe("Jaksel");
    expect(response.body.data.country).toBe("Indonesia");
    expect(response.body.data.postal_code).toBe("12345");
  });

  it("should create address validate error", async () => {
    const contact = await ContactTest.get();
    const response = await supertest(app)
      .post(`/api/contacts/${contact.id}/addresses`)
      .set("X-API-TOKEN", "test")
...
describe("GET /api/contracts/:contractId/addresses/:addressId", () => {
  beforeEach(async () => {
    await UserTest.create();
    await ContactTest.create();
